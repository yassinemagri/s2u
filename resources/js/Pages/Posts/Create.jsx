...
import { Lock, Unlock, FileText, Copy, Check, Sparkles, Gamepad2 } from "lucide-react"
...
const CreatePost = ()=> {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 relative overflow-hidden">
      ...
      <div className="absolute top-8 right-8 font-mono text-xs text-[#FF00FF]/70 z-10">CONTENT CREATOR v1.2.0</div>

      <div className="w-full max-w-2xl space-y-8 relative z-20">
        <div
          className={`text-center space-y-2 ${glitchEffect ? "translate-x-[2px]" : ""} transition-transform duration-75`}
        >
          ...
          >
            Share Your Content
            {glitchEffect && (
              <span className="absolute inset-0 text-[#00FFFF] ml-[3px] mt-[2px] opacity-70">Share Your Content</span>
            )}
          </h1>

          <p className="font-mono text-primary/70">Create a post with locked content that can be unlocked via a link</p>
        </div>
        ...
        <div className="relative">
          ...
          <Card
            className={`rounded-none border-4 border-primary bg-background/90 backdrop-blur-sm p-6 ${glitchEffect ? "translate-x-[1px] translate-y-[1px]" : ""} transition-transform duration-75`}
            style={{ boxShadow: "8px 8px 0px 0px rgba(255,0,255,0.3)" }}
          >
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Post Title */}
              <div className="space-y-2">
                <Label htmlFor="title" className="font-mono text-primary flex items-center">
                  <FileText className="h-4 w-4 mr-2" />
                  Post Title <span className="text-[#FF00FF] ml-1">*</span>
                </Label>
                <Input
                  id="title"
                  name="title"
              ...
              {/* Description */}
              <div className="space-y-2">
                <Label htmlFor="description" className="font-mono text-primary flex items-center">
                  <FileText className="h-4 w-4 mr-2" />
                  Description
                </Label>
                <Textarea
              ...
              {/* Unlock Link */}
              <div className="space-y-2">
                <Label htmlFor="unlockLink" className="font-mono text-primary flex items-center">
                  <Unlock className="h-4 w-4 mr-2" />
                  Unlock Link <span className="text-[#FF00FF] ml-1">*</span>
                </Label>
                <Input
                  id="unlockLink"
                  name="unlockLink"
                ...
                {/* Locked Content */}
              <div className="space-y-2">
                <Label htmlFor="lockedText" className="font-mono text-primary flex items-center">
                  <Lock className="h-4 w-4 mr-2" />
                  Locked Content <span className="text-[#FF00FF] ml-1">*</span>
                </Label>
                <Textarea
                  id="lockedText"
                  name="lockedText"
              ...
              {generatedLink && (
                <div className="mt-6 p-4 border-2 border-[#FF00FF] bg-[#FF00FF]/5">
                  <Label className="font-mono text-primary mb-2 block">Generated Link:</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      readOnly
                      value={generatedLink}
                  ...
                  <p className="font-mono text-xs text-primary/70 mt-2">
                    Share this link with your audience. When they visit, they'll need to visit your unlock link to see
                    the content.
                  </p>
                </div>
              )}
...
CreatePost.layout = (page) => <Layout children={page} />;
